 ...
 * 접근 방법: dp 공부를 위해 해당 알고리즘 공부 후 풀었습니다.
 * 풀이 과정:
 ...
 * - 위 과정을 통해서 규칙을 발견했는데 dp[n] (n>=3)일때 dp[n] = dp[n-1] + dp[n-2]인 것을 확인할 수 있습니다.
 * 1. dp[1], dp[2]를 저장하며, 바텀업 방식으로 dp[n] = (d[n-1] + dp[n-2])%10007을 반복합니다.
